...
const MySkills = () => {
  ...
  return (
    <div id="my-skills" style={{ background: themeState.background }} className="main w-full min-h-screen">
      <div className="pl-5">
        <h1 className="text-3xl pb-1">My Skills</h1>
        <h2 className="font-thin pr-5">
          You're probably wondering,{" "}
          <span className="font-bold">"What can Omar do?"</span> {""}
      ...
      <div className="mx-0">
        <div id="front-end" className="py-5">
          ...
          <div className="grid grid-cols-4"> {frontEndIcons}</div>
        </div>

        <div id="back-end" className=" ">
          <h1 className="text-2xl w-full pl-5">Back-end</h1>
          <div className="grid grid-cols-4">{backEndIcons}</div>
        </div>
      </div>
    </div>
  );
